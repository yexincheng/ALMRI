...
import wandb
from utils import NpzDataset, save_preds_nii, roi_bbox
from torch.utils.data import DataLoader
import argparse

# batch inference
def infer(imgs, embeddings, gts, ckp_path, model_type, device):
...
if  __name__ == '__main__':
    ...
    for p in tqdm(ckp_paths):
        ...
        for t in tqdm(testing_pool):
            ...
            for embeddings, gts, _ in dataloader:
                preds, _ = infer(imgs, embeddings, gts, p, args.sam_model_type, device)
    ...
    np.save(os.path.join(save_path_dice, f'{args.strategy}_{args.task}_{args.base_model}_{args.sam_model_type}_epochs{args.num_epochs}_seed{args.seed}_{args.mode}_dice.npy'), dice_test)
    # plot dice curve
